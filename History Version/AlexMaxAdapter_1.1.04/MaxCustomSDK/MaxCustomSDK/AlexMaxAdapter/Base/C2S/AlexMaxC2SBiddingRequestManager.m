...
- (void)startLoadNativeAdWithRequest:(AlexMaxBiddingRequest *)request {
    ...
    maxNativeAdLoader.nativeAdDelegate = (AlexMaxNativeDelegate *)request.nativeNewCustomEvent;
    maxNativeAdLoader.revenueDelegate = (AlexMaxNativeDelegate *)request.nativeNewCustomEvent;
...
+ (void)disposeLoadSuccessCall:(NSString *)priceStr customObject:(id)customObject unitID:(NSString *)unitID {
    ...
    if ([customObject isKindOfClass:[MAAd class]]) {
        request.nativeMaxAd = customObject;
    }
...
+ (NSString *)handleRateForBidInfo:(ATBidInfo *)bidInfo {
    ...
    if (bidInfo.currencyType == ATBiddingCurrencyTypeCNY || bidInfo.networkFirmID == ATAdNetWorkKuaiShouType) { //CNY-point
        ...
        if (![c2uRateDecimal isEqualToNumber:NSDecimalNumber.notANumber] &&
            ![c2uRateDecimal isEqualToNumber:[NSDecimalNumber zero]]) {
