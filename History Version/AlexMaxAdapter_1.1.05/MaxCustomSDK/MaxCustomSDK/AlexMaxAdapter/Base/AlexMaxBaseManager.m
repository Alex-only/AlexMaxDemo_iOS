...
+ (void)setCommonSetting:(NSDictionary * _Nonnull)serverInfo localInfo:(NSDictionary *)localInfo {
    ...
    //为了在core 93版本修复该问题，暂用kvc取值 date：26-1-9
    if ([[ATSDKGlobalSetting sharedManager] valueForKey:@"hasSetMute"]) {
        [ALSdk shared].settings.muted  = [ATSDKGlobalSetting sharedManager].isMute;
        NSLog(@"%@",[NSString stringWithFormat:@"[ATAdapter][MAX] set Ad mute:%@", [ATSDKGlobalSetting sharedManager].isMute?@"YES":@"NO"]);
    }
...
+ (void)callMaxInitApi:(NSDictionary * _Nonnull)serverInfo unitGroupModel:(ATUnitGroupModel *)unitGroupModel initConfig:(ALSdkInitializationConfiguration *)initConfig {
    //开启一个定时器去轮训查看初始化状态
    [AlexMaxBaseManager sharedManager].checkStatusTimer = [NSTimer scheduledTimerWithTimeInterval:1.0f target:self selector:@selector(checkSDKInitStatus) userInfo:nil repeats:YES];
    // Initialize the SDK with the configuration
    [[ALSdk shared] initializeWithConfiguration:initConfig completionHandler:^(ALSdkConfiguration *sdkConfig) {
        [self destoryCheckStatusTimer];
...
#pragma mark - 检查初始化状态
/* 销毁定时器 */
+ (void)destoryCheckStatusTimer {
    [[AlexMaxBaseManager sharedManager].checkStatusTimer invalidate];
    [AlexMaxBaseManager sharedManager].checkStatusTimer = nil;
   
}
+ (void)checkSDKInitStatus {
    BOOL status = [ALSdk shared].initialized;
    if (status) {
        [self completeInitBlock];
        [self destoryCheckStatusTimer];
    }
