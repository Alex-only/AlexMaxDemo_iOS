...
- (UIView *)getNetWorkMediaView{
    ...
    if ([self isMaxAdTemplateType] && view) {
        return nil;
    }
...
- (void)slefRenderRenderOffer:(ATNativeADCache * _Nonnull)offer {
    ...
    if ([self.ADView respondsToSelector:@selector(dislikeButton)] && self.ADView.dislikeButton) {
        UIButton *closeBtn = self.ADView.dislikeButton;
        
        CGRect closeFrame = closeBtn.frame;
        [closeBtn removeFromSuperview];
        [self.ADView addSubview:closeBtn];
        [closeBtn mas_remakeConstraints:^(MASConstraintMaker *make) {
            make.left.equalTo(self.ADView).offset(closeFrame.origin.x);
            make.top.equalTo(self.ADView).offset(closeFrame.origin.y);
            make.size.mas_equalTo(closeFrame.size);
        }];
        [self.ADView bringSubviewToFront:closeBtn];
    }
}
...
- (BOOL)isMaxAdTemplateType {
    ...
    ATNativeADCache *offer = (ATNativeADCache*)self.ADView.nativeAd;
    MANativeAdView *view = offer.assets[kAlexMAXNativeAssetsExpressAdViewKey];
    if ([self.customEvent.serverInfo[@"unit_type"] integerValue] == AlexMaxNativeRenderTypeTemplate && view) {
        return YES;
    }
    return NO;
}
