...
- (void)callMaxInitApiConfig:(ALSdkInitializationConfiguration *)initConfig {
    ...
    __weak typeof(self) weakSelf = self;
    //开启一个定时器去轮训查看初始化状态
    self.checkStatusTimer = [NSTimer scheduledTimerWithTimeInterval:1.0f repeats:YES block:^(NSTimer * _Nonnull timer) {
        __strong __typeof__(self) strongSelf = weakSelf;
        [strongSelf checkSDKInitStatus];
    }];
    [[ALSdk shared] initializeWithConfiguration:initConfig completionHandler:^(ALSdkConfiguration *sdkConfig) {
        [weakSelf destoryCheckStatusTimer];
